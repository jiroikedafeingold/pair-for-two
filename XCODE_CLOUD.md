...
- **Real bundle identifier** — `com.feingold5.pairfortwo` (was a `devplaceholder…` value).
...
- **Archivable Release config** — the scheme's Archive action uses the Release configuration.
...
## 1. Register the App in App Store Connect  (~5 min)

1. Go to <https://developer.apple.com/account/resources/identifiers> → **Identifiers → +** →
   **App IDs → App**. Register **`com.feingold5.pairfortwo`** (description "Pair for Two"), enable no
   special capabilities (the app needs none beyond Local Network, which is Info.plist-only).
2. Go to <https://appstoreconnect.apple.com> → **Apps → + → New App**:
   - Platform: iOS
   - Name: **Pair for Two**
   - Primary language, and **Bundle ID: com.feingold5.pairfortwo**
   - SKU: anything unique, e.g. `pairfortwo`.
